...
class KVQ_VideoClsDataset(Dataset):
    """Load your own video classification dataset."""

    def __init__(self, data_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        if self.mode in ['train']:
            self.aug = True
            if self.args and self.args.reprob > 0:
    ...
    def _construct_loader(self):
        """Construct the video loader."""
        if self.mode == 'train':
            path_to_file = os.path.join(self.data_path, 'train_split_v4_revise66_compete.txt')
        else:
            # path_to_file = os.path.join(self.data_path, 'test_split_v4_revise66_compete.txt')
            path_to_file = os.path.join(self.data_path, 'val_split_v4_revise66_compete.txt')
        ...
        with open(path_to_file, "r") as f:
            lines = f.readlines()
            for line in lines:
                filename, cls_label, dis_label, label = line.strip().split(',')

                for idx in range(self.num_segment if self.mode != 'test' else self.test_num_segment):
                    self._path_to_videos.append(os.path.join(self.data_path, filename))
                    self._labels.append(float(label) / 5.0)
                    self._spatial_temporal_idx.append(idx)
                    clip_idx += 1

        assert len(self._path_to_videos) > 0, f"Failed to load dataset from {path_to_file}"
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            return buffer, self._labels[index], index, {}
            ...
            return buffer, self._labels[index], sample.split("/")[-1].split(".")[0]
            ...
            # buffer = self.data_transform(buffer)
            # # return buffer, self.test_labels[index], sample.split("/")[-1].split(".")[0], \
            # #        chunk_nb, split_nb
