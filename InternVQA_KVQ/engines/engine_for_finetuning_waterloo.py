...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        targets = targets.to(device, non_blocking=True).to(torch.bfloat16)
...
def validation_one_epoch(data_loader, model, device, ds=False, bf16=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        target = target.to(device, non_blocking=True).to(torch.bfloat16)
...
def validation_one_epoch_vqa(data_loader, model, device, ds=False, bf16=False):
    ...
    gt_labels = torch.cat([r["gt_label"] for r in results], dim=0)
    pr_labels = torch.cat([r["pr_labels"] for r in results], dim=0)

    if is_ddp:
        # Create placeholders for all-gather operation
        gather_list_gt = [torch.zeros_like(gt_labels) for _ in range(dist.get_world_size())]
        gather_list_pr = [torch.zeros_like(pr_labels) for _ in range(dist.get_world_size())]
        dist.all_gather(gather_list_gt, gt_labels)
        dist.all_gather(gather_list_pr, pr_labels)
        gt_labels = torch.cat(gather_list_gt, dim=0)
        pr_labels = torch.cat(gather_list_pr, dim=0)

    gt_labels = gt_labels.cpu().float().numpy()
    pr_labels = pr_labels.cpu().float().numpy()
    pr_labels = rescale(pr_labels, gt_labels)
    print('gt:',gt_labels.shape)
    print('pr:',pr_labels.shape)
    s = spearmanr(gt_labels, pr_labels)[0]
    p = pearsonr(gt_labels, pr_labels)[0]

    metric_logger.update(s=s)
    metric_logger.update(p=p)
    metric_logger.synchronize_between_processes()
    print('* loss {losses.global_avg:.3f}'.format(losses=metric_logger.loss))
...
def final_test_vqa(data_loader, model, device, output_csv_file, ds=False, bf16=False):
    import csv
    ...
    for batch in utils.MetricLogger(delimiter="  ").log_every(data_loader, 10, header):
        # print(batch)
        videos = batch[0]
        # print(videos.shape)
        sample_path = batch[1]  # 对应于每个样本的唯一 ID
        sample_name = sample_path[0].split('/')[-2] +'/'+sample_path[0].split('/')[-1]
        # 数据迁移到设备
        videos = videos.to(device, non_blocking=True)
        ...
        for i in range(output.size(0)):
            results.append({
                "filename": sample_name,  # 保留 ID 的最后三位字符
                "score": float(output[i].detach().float().cpu().numpy())  # 分数保留两位小数
            })
    ...
    print('*Results have been successfully written to:', output_csv_file)
