...
from engines.engine_for_finetuning_waterloo import train_one_epoch_vqa, validation_one_epoch_vqa, final_test_vqa, merge
...
def get_args():
    ...
    parser.add_argument('--data_set', default='Waterloo4K', choices=[
        'KVQ'
...
def main(args, ds_init):
    ...
    if args.disable_eval_during_finetuning:
        ...
        dataset_val, _ = build_dataset(is_train=False, test_mode=True, args=args)
    ...
    print('Trainable number of params:', n_parameters/1e6)
    print(f"Total Model parameters: {total_parameters/1e6:.2f} M")  # in millions
    ...
    if args.eval:
        preds_file = os.path.join('Test_output/', args.log_dir.split('/')[-1] + '.csv')
        # test_stats = final_test_vqa(data_loader_test, model, device, preds_file, ds=args.enable_deepspeed, bf16=args.bf16)
        test_stats = final_test_vqa(data_loader_val, model, device, preds_file, ds=args.enable_deepspeed, bf16=args.bf16)
